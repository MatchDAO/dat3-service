...
public class RedisUtil {
    ...
    public boolean hSetExpire(String key, Long exp, TimeUnit timeUnit) {
        try {

            return redisTemplate.opsForHash().getOperations().expire(key,exp,timeUnit) ;
        } catch (Exception e) {
            log.error(key, e);
            return false;
        }
    ...
    public Boolean getBit(String key, long offset) {
        try {
            Boolean remove = redisTemplate.opsForValue().getBit(key,offset);
            return remove;
        } catch (Exception e) {
            log.error(key, e);
            return true;
